frappe.ui.form.on('Payment Entry', {
    ...
    before_cancel: async function (frm) {
        ...
        await cancel_advance(advance_id)
        
    },
    before_discard: async function (frm) {
        let references = frm.doc.references;
        let advance_id = null; 
...
async function cancel_advance(advance_id){
    frappe.call({
        method:"advance.overrides.payment_entry.payment_entry.cancel_advace",
        args:{refresh_id:advance_id},
        freeze: true,
        freeze_message: __("Update advance Please waite..."),
        callback:function(resp){
